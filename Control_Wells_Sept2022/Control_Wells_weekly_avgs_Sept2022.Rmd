---
title: "Control_Wells_weekly_avgs_Sept2022"
...
## Latest Control Well Compilation Data (September 2022)

```{r}

Control_Wells_Sept2022 <- readr::read_csv("raw/compiledwells_Control_Sept2022.csv", 
...
Control_Wells_good_data_Sept2022 <- Control_Wells_Sept2022 %>%
...
weekly_Control_Wells_toSept2022 <- temp_agg_meadow_dat(Control_Wells_good_data_Sept2022, interval = "30 min", start_day_of_week = 7) %>% 
  mutate(date = as.character(date)) %>%
  dplyr::rename(Date = date) %>%
...
weekly_Control_Wells_toSept2022 %>% 
  readr::write_csv(path = "weekly_Wells_Control_Sept2022.csv")
