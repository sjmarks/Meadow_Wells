...
* Need to finish `process_well_data` function so it updates already compiled file (i.e. performs the coalecse join)- added needs testing
* Need to implement writing to csv file built into the `process_well_data` functions- added needs testing
* Currently everything works well with creating the compiled file from scratch. . . only funky behavior is sometimes files that were corrected for barometric using HoboWare have inconsistent no. of columns which prompts a warning from `readr::read_csv`. As it is right now `clean_hobo_u20` expects 10 columns from this file type. Visit here for possible solution? - link[https://community.rstudio.com/t/skip-columns-and-specify-column-types-in-readr-without-warnings/25019/3]
* All depths are reported to three decimal places
...
clean_hobo_u20 <- function(path){
  ...
  if(stringr::str_detect(path, "hobocorrected")){
    ...
    ID <- readr::read_csv(path, n_max = 1, col_names = FALSE) %>% 
      dplyr::pull(var = X1) %>%
      as.character() %>%
    ...
    data <- readr::read_csv(path, skip = 2, 
                       col_types = "_cdddd____", col_names = c("Date", "total.pressure_psi", "temp.ref_F", "baro.pressure_psi", "water.level.above.sensor"))
...
## RC Test Compile up to July 2020

```{r}

test_compile_RC <- process_well_data(path = "RC_Wells_July2020/raw", site = "RC")
...
test_compile_RC %>% 
  dplyr::mutate(Date = as.character(Date)) %>% 
  readr::write_csv(path = paste0("RC_Wells_July2020/raw", "/", "compiledwells_RC_July2020.csv"))

```

## Marian Test Compile up to July 2020

```{r}

test_compile_Marian <- process_well_data(path = "Marian_Wells_July2020/raw", site = "Marian")
...
test_compile_Marian %>% 
  dplyr::mutate(Date = as.character(Date)) %>% 
  readr::write_csv(path = paste0("Marian_Wells_July2020/raw", "/", "compiledwells_Marian_July2020.csv"))

```

## Control Test Compile up to July 2020

```{r}

test_compile_Control <- process_well_data(path = "Control_Wells_July2020/raw", site = "Control")

## We good
test_compile_Control %>% 
  dplyr::filter(duplicated(Date) | duplicated(Date, fromLast = TRUE)) %>% 
  dplyr::arrange(Date)
...
test_compile_Control %>% 
  dplyr::mutate(Date = as.character(Date)) %>% 
  readr::write_csv(path = paste0("Control_Wells_July2020/raw", "/", "compiledwells_Control_July2020.csv"))
