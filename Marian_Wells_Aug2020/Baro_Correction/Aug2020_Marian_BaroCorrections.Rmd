---
title: "Aug2020_Marian_BaroCorrections"
author: "Simon Marks"
date: "9/23/2020"
...
Aug_2020_ATM_Marian <- read_csv("Marian_Wells_Aug2020/Baro_Correction/atm_control_Aug2020.csv", skip = 2, 
  ...
  dplyr::filter(Date > as.POSIXct("2020-07-07 09:30:00", tz = "UTC"))
...
Aug_2020_ATM_Marian_revised <- dplyr::bind_rows(Aug_2020_ATM_Marian, Aug_2020_ATM_RC)

#################################################################################################################
## Only deep wells were collected during this trip MM3W and MM7W

## CMW0 collected during 08/2020 trip

Aug2020_MM3W_uncorrected <- read_csv("Marian_Wells_Aug2020/Baro_Correction/MM3W_Aug2020_uncorrected.csv", skip = 2, 
...
Aug2020_MM7W_uncorrected <- read_csv("Marian_Wells_Aug2020/Baro_Correction/MM7W_Aug2020_uncorrected.csv", skip = 2, 
                                     col_types = "_???_____", col_names = c("Date", "total.pressure_psi", "temp.ref_F")) %>% 
  dplyr::mutate(Date = lubridate::mdy_hms(Date)) %>% 
  dplyr::filter(Date <= as.POSIXct("2020-08-17 09:00:00", tz = "UTC"))
...
MM3W_Aug2020_corrected <- barometric_datafile_correction(Aug2020_MM3W_uncorrected, Aug_2020_ATM_Marian_revised)

MM3W_Aug2020_corrected %>% 
  dplyr::mutate(Date = as.character(Date)) %>%
  dplyr::mutate(ID = "MM3W") %>%
  readr::write_csv(path = "Marian_Wells_Aug2020/Baro_Correction/MM3W_Aug2020_corrected.csv")

## MM7W Aug 2020

MM7W_Aug2020_corrected <- barometric_datafile_correction(Aug2020_MM7W_uncorrected, Aug_2020_ATM_Marian_revised)

MM7W_Aug2020_corrected %>% 
  dplyr::mutate(Date = as.character(Date)) %>%
  dplyr::mutate(ID = "MM7W") %>%
  readr::write_csv(path = "Marian_Wells_Aug2020/Baro_Correction/MM7W_Aug2020_corrected.csv")
