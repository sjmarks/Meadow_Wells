---
title: "Marian_Wells_weekly_avgs_Jun2022"
...
## Latest 30-minute Marian Well Compilation Data (June 2022)

```{r}

Marian_Wells_Jun2022 <- readr::read_csv("raw/compiledwells_Marian_Jun2022.csv", 
...
Marian_Wells_good_data_Jun2022 <- Marian_Wells_Jun2022 %>%
...
weekly_Marian_Wells_toJun2022 <- temp_agg_meadow_dat(Marian_Wells_good_data_Jun2022, interval = "30 min", start_day_of_week = 7) %>% 
  mutate(date = as.character(date)) %>%
  rename(Date = date) %>%
...
weekly_Marian_Wells_toJun2022 %>% 
  # Date variable is already recognized as a character variable
  readr::write_csv(path = "weekly_Wells_Marian_Jun2022.csv")
