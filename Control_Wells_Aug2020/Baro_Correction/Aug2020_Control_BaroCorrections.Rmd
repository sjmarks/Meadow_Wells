---
title: "Aug2020_Control_BaroCorrections"
author: "Simon Marks"
date: "9/23/2020"
...
## Atm correction file (most recent control meadow file)- control meadow atm stopped logging on 2020-07-19 04:30:00 due full memory

Aug_2020_ATM_Control <- read_csv("Control_Wells_Aug2020/Baro_Correction/atm_control_Aug2020.csv", skip = 2, 
                                 col_types = "_cdd____", col_names = c("Date", "baro.pressure_psi", "baro.temp_F")) %>%
  dplyr::mutate(Date = lubridate::mdy_hms(Date)) %>% 
  dplyr::filter(Date > as.POSIXct("2020-07-07 15:30:00", tz = "UTC"))

## Atm correction file (most recently RC file)

#################################################################################################################

Aug_2020_ATM_RC <- read_csv("Control_Wells_Aug2020/Baro_Correction/RC_ATM_Aug2020.csv", skip = 2,
                            col_types = "_cdd____", col_names = c("Date", "baro.pressure_psi", "baro.temp_F")) %>%
  dplyr::mutate(Date = lubridate::mdy_hms(Date)) %>% 
...
Aug_2020_ATM_Control_revised <- dplyr::bind_rows(Aug_2020_ATM_Control, Aug_2020_ATM_RC)

#################################################################################################################
## Only deep wells were collected during this trip CMW1 and CMW0

## CMW0 collected during 08/2020 trip

Aug2020_CMW0_uncorrected <- read_csv("Control_Wells_Aug2020/Baro_Correction/CM0W_Aug2020_uncorrected.csv", skip = 2, 
                                     col_types = "_???_____", col_names = c("Date", "total.pressure_psi", "temp.ref_F")) %>% 
  dplyr::mutate(Date = lubridate::mdy_hms(Date)) %>% 
  dplyr::filter(Date > as.POSIXct("2020-07-07 15:30:00:00", tz = "UTC") & Date <= as.POSIXct("2020-08-17 09:00:00", tz = "UTC"))
...
## CM4W collected 08/2020 trip

Aug2020_CM4W_uncorrected <- read_csv("Control_Wells_Aug2020/Baro_Correction/CM4W_Aug2020_uncorrected.csv", skip = 2, 
                                     col_types = "_???_____", col_names = c("Date", "total.pressure_psi", "temp.ref_F")) %>% 
  dplyr::mutate(Date = lubridate::mdy_hms(Date)) %>% 
  dplyr::filter(Date > as.POSIXct("2020-07-07 15:30:00", tz = "UTC") & Date <= as.POSIXct("2020-08-17 09:00:00", tz = "UTC"))
...
## CM0W Aug 2020 (bottom deep well)

CM0W_Aug2020_corrected <- barometric_datafile_correction(Aug2020_CMW0_uncorrected, Aug_2020_ATM_Control_revised)

CM0W_Aug2020_corrected %>% 
  dplyr::mutate(Date = as.character(Date)) %>%
  dplyr::mutate(ID = "CM0W") %>%
  readr::write_csv(path = "Control_Wells_Aug2020/Baro_Correction/CM0W_Aug2020_corrected.csv")

## CM4W Aug 2020 (top deep well)

CM4W_July2020_corrected <- barometric_datafile_correction(Aug2020_CM4W_uncorrected, Aug_2020_ATM_Control_revised)

CM4W_July2020_corrected %>% 
  dplyr::mutate(Date = as.character(Date)) %>%
  dplyr::mutate(ID = "CM4W") %>%
  readr::write_csv(path = "Control_Wells_Aug2020/Baro_Correction/CM4W_Aug2020_corrected.csv")
