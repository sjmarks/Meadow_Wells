...
## CM1W collected 08/2020 trip

Aug2020_CM1W_uncorrected <- read_csv("Control_Wells_Aug2020/Baro_Correction/CM1W_Aug2020_uncorrected.csv", skip = 2, 
                                     col_types = "_???_____", col_names = c("Date", "total.pressure_psi", "temp.ref_F")) %>% 
  dplyr::mutate(Date = lubridate::mdy_hms(Date)) %>% 
...
CM1W_July2020_corrected <- barometric_datafile_correction(Aug2020_CM1W_uncorrected, Aug_2020_ATM_Control_revised)

CM1W_July2020_corrected %>% 
  dplyr::mutate(Date = as.character(Date)) %>%
  dplyr::mutate(ID = "CM1W") %>%
  readr::write_csv(path = "Control_Wells_Aug2020/Baro_Correction/CM1W_Aug2020_corrected.csv")
